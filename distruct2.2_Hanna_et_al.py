#################
################# This is distruct2.2.py by Vikram E. Chhatre, University of Vermont
...
#with open('poporder') as f:
#    poporder = [x.strip('\n') for x in f.readlines()]

#def get_poporder(params):
#    if params.has_key('poporder'):
...
#    else:
#	print "If you'd like to arrange populations in a desired order, include the --poporder flag"
...
def plot_admixture(admixture, population_indices, population_labels, title):
    ...
    figure = plot.figure(figsize=(8,2)) #You can change plot dimensions here
    ...
    for k in xrange(K):
        ...
        subplot.bar(lefts, admixture[:,k], align='edge', width=indiv_width, bottom=bottoms, facecolor=colors[k], edgecolor='#000000', linewidth=0.1)
    ...
    for p,popname in enumerate(population_labels):
        ...
        if indices.size>0:
            ...
            subplot.axvline(vline_pos, linestyle='-', linewidth=0.3, c='#000000')
...
def get_admixture_proportions(params):
    ...
    if params.has_key('poporder') & params.has_key('popfile'):
...
if __name__=="__main__":
    ...
    figure.savefig(params['outputfile'], dpi=300, bbox_inches='tight', pad_inches=0)
